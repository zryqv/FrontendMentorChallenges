...
function App() {
  ...
  }, [watch("number"), done]); // a very ugly way to add space between each four numbers i'm sure there are simpler solutions  but this is the first thing that came to mind and I didn't want to waste a lot of time here
  const onSubmit = (data) => {
    console.log(data);
    setDone(true);
  };
  return (
    <div className="h-screen w-screen bg-gradient-to-r from-violet-500 to-fuchsia-50 flex justify-center items-center">
      <div className="sm:w-[1440px]  sm:h-[900px]  flex flex-col sm:flex-row w-screen h-screen relative">
        ...
        {/* card front */}
        <div className="absolute text-white left-[4%] top-[12.4%] sm:top-[27%] md:left-[9%] lg:left-[16%] lg:top-[17%] md:top-[21%]">
          <div className="relative w-72 sm:w-[20rem] md:w-[25rem]  lg:w-fit">
        ...
        {done ? (
          <ThankYou
            className="lg:-mr-[14rem] lg:w-[28vw] xl:w-[23vw] sm:w-[40vw] sm:-mr-[1.5rem] md:w-[30vw] md:-mr-[7rem]"
            setDone={setDone}
          />
        ) : (
          <>
